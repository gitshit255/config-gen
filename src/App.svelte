<script>
  ...
  import { normalizePatchesData } from "./lib/patchesFormat.js";
...
<main>
  ...
  <br />
  A plain JSON array of patch names is also accepted, e.g. <code>["NewX: Remove ads", ...]</code>. Names sharing an "App: " prefix are grouped into one app card; descriptions, versions, and options won't be available since that format doesn't include them.
  </div>
  {/if}
    ...
    <input
      ...
      on:change={(e) => {
        ...
        r.onload = (re) => {
          ...
          defaultPatchesJson = Promise.resolve(normalizePatchesData(j));
          TOML = [];
        };
      }}
      id="files"
